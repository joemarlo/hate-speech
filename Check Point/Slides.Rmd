...
Collect a random sample of 250,000 United States Twitter users
- Use random number generation to identify 250,000 unique twitter users
...
- Obtaining 250,000 users in the United States will require ~50 million random integers 

- To bypass twitter API call limits, we can only make 900 API calls every 15 min. 

- In order to obtain the sample, estimates suggest we will need 15 days of continuous API processing. 

- To run days of continuous calls, we are using Raspberry Pi.
...
<img src="Img/tweets_over_time.png"/>  
